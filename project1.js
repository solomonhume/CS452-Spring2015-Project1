...
var map = [ // 1  2  3  4  5  6  7  8  9
           ...
           [1, 1, 1, 0, 0, 1, 0, 0, 1, 1,], // 8
           [1, 1, 1, 1, 1, 1, 0, 0, 0, 1,], // 9
...
var runAnim = true, mouse = { x: 0, y: 0 }, kills = 0, health = 5;
...
function init() {
	...
	// Camera rotates with mouse, moves with WASD/arrow keys
	...
	renderer.domElement.style.backgroundColor = '#D6FFFF'; 
	...
	$(document).click(function(e) {
		e.preventDefault;
		if (e.which === 1) { createBullet();}
	});
	
	// Display HUD
	$('body').append('<canvas id="radar" width="200" height="200"></canvas>');
	$('body').append('<div id="hud"><p>Health: <span id="health">5</span><br />Score: <span id="score">0</span></p></div>');
	$('body').append('<div id="credits"><p>Created by Jiaju Huang</a> <br />WASD to move, mouse to turn around, click to shoot</p></div>');
...
function render() {
	...
	for (var i = bullets.length-1; i >= 0; i--) {
		...
		for (var j = ai.length-1; j >= 0; j--) {
			...
			if (p.x < c.x + x && p.x > c.x - x &&
				...
				var color = a.material.color, percent = a.health / 5;
		...
		if (distance(p.x, p.z, cam.position.x, cam.position.z) < 25 && b.owner != cam) {
			...
			val = health < 3 ? '<span style="color: darkRed">' + health + '</span>' : health;
			$('#health').html(val);
			bullets.splice(i, 1);
			scene.remove(b);
			$('#hurt').fadeOut(350);
	...
	if (health <= 0) {
		...
		$('#radar, #hud, #credits').fadeOut();
		$('#intro').fadeIn();
		$('#intro').html('You lose! Click to restart...');
...
function setupScene() {
	...
	var floor = new t.Mesh(
			...
			new t.MeshLambertMaterial({color: 0xEDD0A0,})
	);
	scene.add(floor);
	...
	var materials = [
	                 new t.MeshLambertMaterial({color: 0x00CCFF,}),
...
// Pause game when the window is unfocused
